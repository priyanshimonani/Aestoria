function search(category) {
  ...
  fetch(`https://api.harvardartmuseums.org/object?apikey=a3516175-d7de-4012-9a7c-73df61ff62c5&size=20&classification=${category}`)
        ...
        data.records.forEach(item => {
          if (item.primaryimageurl) {
            ...
            image.innerHTML = `
              ...
              <img src="${item.primaryimageurl}" loading="lazy" width="300" style="margin:auto; border:1px solid #ccc; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.2); max-height: 500px;"/>
              <h3>${item.title}</h3>
              <h3>${artist}</h3>
